<template>
  <div class="cart-wrapper">
    ...
    >
      <img
        src="../../../static/images/cart_empty@2x.png"
        alt="提示背景"
        mode="aspectFill"
      >
      ...
      <scroll-view scroll-y class="scroll-view">
        ...
        <div class="address-wrapper">
          ...
          >
            ...
            <div class="consignee-address">
              收货地址: <span class="address">{{addressData.addres}}</span>
            </div>
          </div>
          <button
            plain
        ...
        <div class="cart-wrapper">
          ...
          <div class="cart-content">
            <div
              class="cart-list"
              v-for="(list, key) in cartData"
              :key="key"
            >
              <div
                class="select-icon iconfont icon-xuanze"
                @click="handleSelectGood(list)"
                :class="list.isSelected?'hasSelected':''"
              ></div>
              <div class="good-wrapper">
                <navigator
                  :url="'/pages/detail/main?goodsId='+list.goods_id"
                  class="good-img"
                >
                  <img
                    :src="list.goods_small_logo"
                    mode="aspectFill"
                    alt=""
                  >
                </navigator>
                <div class="good-info">
                  ...
                  <div class="tool-wrapper">
                    ...
                    <div class="tool">
                      <div
                        class="reduce iconfont icon-wuuiconsuoxiao"
                        @click="reducePayCount(list)"
                      ></div>
                      <div class="count">{{list.buyCount}}</div>
                      <div
                        class="add iconfont icon-wuuiconxiangjifangda"
                        @click="addPayCount(list)"
                      ></div>
                    </div>
                  </div>
                </div>
        ...
        <div class="tool-left">
          <div class="select-all">
            <div
              ...
              :class="hasSelectedAll?'hasSelected':''"
            ></div>
...
export default {
  ...
  methods: {
      ...
      wx.chooseAddress({
        success: res => {
          ...
          this.addressData = {
            ...
            addres: `${res.provinceName}${res.cityName}${res.countyName}${
              res.detailInfo
            }`,
    ...
    handleSelectGood(list) {
      this.cartData[list.goods_id].isSelected = !this.cartData[list.goods_id]
        .isSelected;
    },
    // 是否全选
    handleSelectAllGood() {
    ...
    handleToPay() {
      ...
      const payData = {
        ...
        consignee_addr: JSON.stringify(this.addressData),
        goods: goods
      };
      console.log(payData)
      // 发送请求
    ...
    onGotUserInfo(event) {
      ...
      // }
      // // 发送请求
      // request.auth('https://itjustfun.cn/api/public/v1/my/orders/create', payData)
      //   .then(res => {
      //     console.log(res)
      //   })
      // const {detail} = event.mp
      // console.log(detail)
      // wx.setStorageSync('userToken', detail)
    }
  },
  computed: {
    // 判断是否存在本地地址
